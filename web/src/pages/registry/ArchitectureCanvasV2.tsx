...
function pointsAttr(pts: Point[]): string { return pts.map(p => `${p.x},${p.y}`).join(' ') }
...
function loadColors(): Record<string, string> {
  try { return JSON.parse(localStorage.getItem('arch_edge_colors_v2') ?? '{}') } catch { return {} }
}
function saveColors(colors: Record<string, string>) {
  localStorage.setItem('arch_edge_colors_v2', JSON.stringify(colors))
}
...
export default function ArchitectureCanvasV2({ systems, edges, config, statusConfigs, onNodeMoved, onOpenRegistry }: Props) {
  ...
  const onBgMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    setPopup(null); setColorPickKey(null)
  ...
  const onMouseUp = useCallback((_e: React.MouseEvent) => {
    ...
    drag.current = null; panRef.current = null
  }, [pos, onNodeMoved])

  const onNodeMouseDown = useCallback((e: React.MouseEvent, id: string) => {
    e.stopPropagation(); setPopup(null); setColorPickKey(null)
  ...
  const onEdgeClick = useCallback((ev: React.MouseEvent, edge: DiagramEdge) => {
    ...
    if (!rect) return
    setPopup({
      sx: ev.clientX - rect.left, sy: ev.clientY - rect.top, edge,
      senderName:   systems.find(s => s.id === edge.sender_system_id)?.name   ?? '?',
      receiverName: systems.find(s => s.id === edge.receiver_system_id)?.name ?? '?',
    })
  ...
  return (
    ...
    >
      {/* Toolbar */}
      <div style={{ position: 'absolute', top: 8, left: 8, zIndex: 20, display: 'flex', gap: '6px', alignItems: 'center' }}
        onMouseDown={e => e.stopPropagation()}>
        <button style={toolBtn} onClick={fitView}>⤢ Fit</button>
        <select value={layoutAlgo} onChange={e => setLayoutAlgo(e.target.value as LayoutAlgo)}
          style={{ ...toolBtn, cursor: 'pointer' }} onMouseDown={e => e.stopPropagation()}>
          {LAYOUT_OPTIONS.map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
        </select>
      </div>

      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', overflow: 'visible' }} onMouseDown={onBgMouseDown}>
        <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
          <defs>
            <marker id="arrow-v2" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
          ...
          {/* Pass 2: count badge — always on top */}
          {edges.map(edge => {
            const pts = getEdgePts(edge)
            if (pts.length < 2) return null
            const key   = edgeKey(edge)
            ...
            return (
              <g key={`lbl-${key}`} style={{ pointerEvents: 'none' }}>
                <g transform={`translate(${mid.x},${mid.y})`}>
                  ...
                  <text textAnchor="middle" dominantBaseline="middle" fontSize={11} fill="#fff" fontFamily="var(--sapFontFamily)">
                    {edge.count}
                  </text>
                </g>
              </g>
      ...
      {/* Edge colour picker */}
      {colorPickKey && (
        <div style={{ position: 'absolute', top: 60, left: 8, zIndex: 100, background: 'var(--sapTile_Background)', border: '1px solid var(--sapList_BorderColor)', borderRadius: '6px', padding: '8px', display: 'flex', gap: '6px' }}
          onMouseDown={e => e.stopPropagation()}>
          {PALETTE.map(c => (
            <div key={c}
              ...
              onClick={() => { const next = { ...edgeColors, [colorPickKey]: c }; setEdgeColors(next); saveColors(next); setColorPickKey(null) }}
            />
          ))}
          <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#888', cursor: 'pointer' }}
            onClick={() => { const { [colorPickKey]: _, ...rest } = edgeColors; setEdgeColors(rest); saveColors(rest); setColorPickKey(null) }}
          />
        </div>
      )}
      ...
      {popup && (
        <div style={{
          position: 'absolute',
          left: Math.min(popup.sx + 10, (containerRef.current?.clientWidth ?? 600) - 300),
          top:  Math.min(popup.sy + 10, (containerRef.current?.clientHeight ?? 400) - 220),
          zIndex: 100, minWidth: 220, maxWidth: 300,
          background: 'var(--sapTile_Background)', border: '1px solid var(--sapList_BorderColor)',
          borderRadius: '8px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)', padding: '12px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '10px', maxHeight: '160px', overflowY: 'auto' }}>
            {popup.edge.refs.map((ref, i) => (
              <div key={ref} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ padding: '1px 6px', borderRadius: '8px', fontSize: '0.68rem', background: statusColor(popup.edge.statuses[i]), color: '#fff', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <button style={{ ...toolBtn, background: 'var(--sapHighlightColor)', color: '#fff', border: 'none', flex: 1 }}
          ...
          <button style={{ ...toolBtn, marginTop: '6px', width: '100%', fontSize: '0.7rem' }}
