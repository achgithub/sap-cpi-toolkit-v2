...
const systemCols = `id, name, system_type, infra_type, infra_region, description, owner_type, managed_by, deployment_type, owner, pos_x, pos_y, created_at, updated_at`
...
func (h *Handler) createSystem(w http.ResponseWriter, r *http.Request) {
	...
	s, err := scanSystem(h.pool.QueryRow(r.Context(),
		`INSERT INTO systems (name, system_type, infra_type, infra_region, description, owner_type, managed_by, deployment_type, owner, pos_x, pos_y)
...
func (h *Handler) updateSystem(w http.ResponseWriter, r *http.Request) {
	...
	s, err := scanSystem(h.pool.QueryRow(r.Context(),
		`UPDATE systems SET
		 ...
		 pos_x=$10, pos_y=$11, updated_at=now()
		 WHERE id=$12
		 RETURNING `+systemCols,
		body.Name, body.SystemType, body.InfraType, body.InfraRegion,
		body.Description, body.OwnerType, body.ManagedBy, body.DeploymentType, body.Owner,
