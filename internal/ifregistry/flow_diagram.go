...
func (h *Handler) getInterfaceV2FlowDiagram(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	var state json.RawMessage
	err := h.pool.QueryRow(r.Context(),
		`SELECT COALESCE(flow_diagram_state, 'null'::jsonb) FROM interfaces_v2 WHERE id=$1`, id,
	).Scan(&state)
	if err != nil {
		if isNotFound(err) {
			apiError(w, 404, "not found")
		...
		h.log.Error("get interface v2 flow diagram", "error", err)
		apiError(w, 500, "internal error")
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func (h *Handler) putInterfaceV2FlowDiagram(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	defer r.Body.Close()
	var body json.RawMessage
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
	...
	_, err := h.pool.Exec(r.Context(),
		`UPDATE interfaces_v2 SET flow_diagram_state=$1, updated_at=now() WHERE id=$2`, body, id,
	)
	if err != nil {
		h.log.Error("put interface v2 flow diagram", "error", err)
		apiError(w, 500, "internal error")
		return
	}
	w.WriteHeader(http.StatusNoContent)
